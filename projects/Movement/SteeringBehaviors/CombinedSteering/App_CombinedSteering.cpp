...
void App_CombinedSteering::Start()
{
	...
	m_pDrunkWander->SetWanderRadius(0.f);
	vector<BlendedSteering::WeightedBehavior> weightedSteeringBehaviors;
	weightedSteeringBehaviors.push_back({ m_pSeek, 1.f });
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	//UI
	{
		...
		ImGui::SliderFloat("Seek",
			&m_pBlendedSteering->GetWeightedBehaviorsRef()[0].weight, 0.f, 2.f, "%.2f"); // ctrl d --> duplicate line to line below
