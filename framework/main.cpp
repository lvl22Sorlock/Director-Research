...
#elif defined(ActiveApp_PathfindingNavMeshGraph)
#include "projects/Movement/Pathfinding/NavMeshGraph/App_NavMeshGraph.h"
#elif defined(ActiveApp_FiniteStateMachine)
...
#elif defined(ActiveApp_AgarioGame_BTIM)
#include "projects/DecisionMaking/InfluenceMaps/App_AgarioGame_BTIM.h"
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		while (!pWindow->ShutdownRequested())
		{
			...
			//Update (Physics, App)
			PHYSICSWORLD->Simulate(elapsed*25);
			pCamera->Update();
			myApp->Update(elapsed*25);
